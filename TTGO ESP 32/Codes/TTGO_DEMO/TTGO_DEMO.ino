...
//        if(SD.exists("wifilog.txt"))
//          SD.remove("wifilog.txt");
        File dataFile = SD.open("/wlog.txt", FILE_WRITE);
        if(!dataFile)
          Serial.println("XXXXX CANNOT OPEN FILE XXXXXXX ");
        for (int i = 0; i < n; ++i) {
            sprintf(buff,
                    "[%d]:%s(%d)",
                    i + 1,
...
//    tft.setRotation(1);
    ...
    tft.pushImage(0, 0,  240, 135, elementz); // change the name "elementz" to your image name as seen in the bmp file.
    espDelay(5000);
